//loading the data
let worldhappinessreport;
 fetch("./datasets/world-happiness-report-2005-2021.csv")
  .then((response) => response.text())
  .then((text) => {
    worldhappinessreport = text.split("\n").map((row) => row.split(","));
    // console.log(worldhappinessreport); 
    filterData(2021);
  })
  .catch((error) => {
...
let selection = `<select name="year">`;
for (let i = 5; i < 22; i++) {
  var year = 2000 + i;
  selection += `<option value="${year}">${year}</option>`;
}
selection += `</select>`;
...
function filterData(year) {
    const headers = worldhappinessreport[0]; // Assuming the first row contains headers
  
    // Find the indices for the columns you want
    const countryIndex = headers.indexOf("Country"); // Adjusted to match header case
    const ladderScoreIndex = headers.indexOf("Ladder score");
    const yearIndex = headers.indexOf("year"); // Adjusted to match header case
  
    // Filter only the columns you want and match the year
    const filteredData = worldhappinessreport.slice(1).filter((row) => {
      const rowYear = parseInt(row[yearIndex]); // Get the year from the row
      // console.log();
      
      return rowYear == year; // Filter for the specified year
    }).map((row) => ({
      country: row[countryIndex],
      ladderScore: parseFloat(row[ladderScoreIndex]),
      year: parseInt(row[yearIndex]),
    }));
  
    console.log(filteredData); // Logs the filtered data for the specified year
  }
...
// initial setup
  ...
  d3.queue()
    ...
    .defer(d3.csv, worldladderscore, function (d) {
